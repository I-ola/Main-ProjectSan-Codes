...
public class Weapon : MonoBehaviour
{
    ...
    [SerializeField] private float bulletDrop = 9.81f;

    private float bulletMaxTime = 3f;

    List<BulletScript> bullets = new List<BulletScript>();
    public AimingAndEquiping.WeaponSlots weaponSlots;
    public AimingAndEquiping.WeaponType weaponType;
        ...
        if (Physics.Raycast(ray, out hitInfo, range, hitMask))
        {
           // Debug.DrawLine(bulletPathRaycast.position, hitInfo.point, Color.red, 2.0f);
            ...
            if (hitBox)
            {
                ...
                hitBox.OnRaycastHit(this, bodypartHit,ray.direction);
            }
        }
        else
        {
